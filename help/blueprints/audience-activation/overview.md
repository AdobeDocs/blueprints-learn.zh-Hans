...
| **[激活到文件和企业流传输目的地](enterprise-destinations.md)** | <ul><li>激活和访问跨企业系统和应用程序的实时客户资料，以提供丰富的情境式客户体验。 </li></ul><ul><li>利用来自实时客户资料的分析和事件启动销售或支持体验。</li></ul> | <ul><li>Adobe Experience Platform</li><li>[!UICONTROL 实时客户数据平台]</li><li>Experience Platform Activation</li><li>Adobe Audience Manager（可选）</li></ul> |
...
* 请注意，批次目的地不支持激活仅限属性的更改事件。完整或增量受众成员资格可以与选定的用于激活的属性一起发送。

### 将批次区段激活到流式目的地

* 支持将批次区段激活到流式目的地。由于用户档案符合批量区段作业的受众成员资格条件，因此可以通过流式激活来激活这些实现。
