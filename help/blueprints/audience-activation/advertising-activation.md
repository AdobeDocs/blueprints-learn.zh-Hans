---
title: 从实时客户数据平台激活社交和广告目标
description: 从实时客户数据平台激活社交和广告目标
...
从多个来源摄取客户数据以构建客户的单个配置文件视图，对这些配置文件进行分段以构建营销和个性化的受众，将这些受众共享到广告网络(如Facebook和Google)以针对这些受众定位和个性化促销活动。
...
1. 在目标配置中将受众传送配置到目标。
   ...
   * 请参阅 [Google客户匹配目标指南](https://experienceleague.adobe.com/docs/experience-platform/destinations/catalog/advertising/google-customer-match.html?lang=zh-Hans) 有关配置Google目标的更多详细信息。
   * 配置目标时，选择要激活到目标的受众。
...
1. 确保目标流程已按预期将受众传送到目标。
   * 检查监控界面，以确认已向受众发送预期数量的用户档案。受众大小应反映激活的用户档案预期数量，同时注意到特定目标(如Facebook和Google)将需要特定字段（如电子邮件哈希标识），如果受众成员的用户档案中不存在，则不会将其激活到目标。
