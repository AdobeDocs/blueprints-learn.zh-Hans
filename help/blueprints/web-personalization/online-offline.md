...
<td class="tg-73oq">将 Edge 上的实时区段评估从 Real-time Customer Data Platform 共享到 Target</td>
    ...
    <td class="tg-73oq">- 实施模式 1，如下所述。<br>- 必须实施 Web/Mobile SDK。<br>- 请注意，当前不提供基于 Mobile SDK 和 API 的实时分段支持<br>- 必须在 Experience Edge 中配置数据流，并启用 Target 和 Experience Platform 扩展，数据流 ID 将在 Target 目标配置中提供。<br>- 必须在 Real-time Customer Data Platform 目标中配置 Target 目标。<br>- 与 Target 集成需要与 Experience Platform 实例具有相同的 IMS 组织。</td> 
  </tr>
  <tr>
...
## 场景 1 和 2 - 将流传输和批次受众实时共享到 Adobe Target
...
## 场景 3 - 通过受众共享服务将流传输和批次受众共享到 Adobe Target 和 Audience Manager
...
1. [请求为 Experience Platform 与 Adobe Target（共享受众）之间的受众共享进行配置](https://www.adobe.com/go/audiences)以将受众从 Experience Platform 共享到 Target。
1. （可选）[使用 Edge 数据流配置 Edge 网络](https://experienceleague.adobe.com/docs/experience-platform/edge/fundamentals/datastreams.html?lang=zh-Hans)（仅集成模式 2 需要此参数，该模式需要将受众共享到 Audience Manager，或通过 Audience Manager 受众或数据扩充受众）。
1. （可选）[在 Real-time Customer Data Platform 中启用 Adobe Target 作为目标](https://experienceleague.adobe.com/docs/experience-platform/destinations/catalog/personalization/adobe-target-connection.html?lang=zh-Hans)以通过受众共享服务和 Audience Manager 将流传输受众和批次受众从 Real-time Customer Data Platform 直接共享到 Edge。
...
### 实施模式 1 - 支持用例场景 1 和 2。使用 Web/Mobile SDK 的 Edge 网络（推荐方案）

将 Edge 网络与 Web/Mobile SDK 结合使用
...
### 实施模式 2 - 支持用例场景 2 和 3。特定于应用程序的 SDK

使用传统的特定于应用程序的 SDK（例如，AT.js 和 AppMeasurement.js）
