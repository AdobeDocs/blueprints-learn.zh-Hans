...
<tbody>
  ...
  <tr>
    ...
    <td class="tg-73oq"> — 必须在RTCDP目标中配置Target目标。<br>- 与 Target 集成需要与 Experience Platform 实例具有相同的 IMS 组织。<br>不需要 WebSDK。<br>- 如果使用 AT.js，则仅支持针对 ECID 的用户档案查找。<br>- 对于 Edge 上的自定义 ID 命名空间查找，需要部署 WebSDK，并且必须在身份映射中将每个身份设置为身份。</td>
  </tr>
  <tr>
    <td class="tg-0lax">3</td>
    <td class="tg-73oq">在使用 WebSDK 通过 Edge 共享到 Target 的 Edge 上进行 RTCDP 实时区段评估。</td>
    <td class="tg-0lax"> - 在 Edge 上实时评估受众以进行同页或下一页个性化。</td>
    <td class="tg-73oq"> — 必须在RTCDP目标中配置Target目标。<br>- 与 Target 集成需要与 Experience Platform 实例具有相同的 IMS 组织。<br>- 必须实施 WebSDK。<br> — 尚未提供基于Mobile SDK和API的实施</td>
  </tr>
</tbody>
</table>
