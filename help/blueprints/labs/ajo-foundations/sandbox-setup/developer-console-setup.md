---
title: Developer Console设置
description: 使用OAuth服务器到服务器凭据为DEP CLI使用的Experience Platform和Journey Optimizer API创建Adobe Developer Console项目。
doc-type: article
...
1. 为凭据提供一个名称并选择&#x200B;**下一步**
1. 选择与您使用的沙盒匹配的产品配置文件，然后选择&#x200B;**保存配置的API**

## 添加Adobe Journey Optimizer API

1. 从项目概述中，选择&#x200B;**添加API**
1. 选择&#x200B;**Adobe Journey Optimizer**&#x200B;产品图标并选择相关的API
1. 选择&#x200B;**OAuth服务器到服务器**
1. 选择相同的产品配置文件，然后选择&#x200B;**保存配置的API**

>[!NOTE]
>
>重复使用您在上面创建的凭据，而不是创建新的凭据 — CLI只需要一组凭据，并且包含组合范围。
...
>[!NOTE]
>
>复制凭据页面上显示的默认范围 — 无需手动添加任何内容。 如果您添加了以上两个API，则范围列表将自动包含这两个API。
