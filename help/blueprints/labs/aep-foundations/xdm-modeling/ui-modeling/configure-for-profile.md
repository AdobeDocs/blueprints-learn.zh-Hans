...
要将Real-time Customer Profile的架构利用，您首先需要确保该架构已正确配置。 此步骤意味着将您在LID实验室中确定的身份作为主/人员身份、关系身份等，并确保对每个架构都进行了这些配置。 完成所有操作后，您启用架构以便与配置文件一起使用。
...
如果要与实时客户档案一起使用，则每个架构都需要主标识字段。 要将字段标记为主要标识，请执行以下步骤。
...
>[!NOTE]
>
>另请注意，您现在在左边栏中看到以下项目。 此处将显示身份（主要或非主要），**主要**&#x200B;身份也标记为必填字段。
>
>
>
...
每个架构都可以&#x200B;**选择性地包含**&#x200B;其他人员标识字段。 此规则适用于与实时客户档案一起使用的任何架构。 要将字段标记为人员标识，请对您之前创建的Customer Account架构执行以下操作。
...
要将Plan架构与ERD中所述的Customer Account架构相关联，您需要定义关系。 要在客户帐户和计划（查找）架构之间创建架构关系，请执行以下步骤。
...
1. 在类型选择框中，选择&#x200B;**一对一**&#x200B;选项
1. 在“引用架构”选择框中，选择名为&#x200B;**dep： Plan \[Lookup]**&#x200B;的架构（已为您预创建此架构）
1. 单击&#x200B;**应用**&#x200B;和&#x200B;**保存**
...
如前所述，XDM和实时客户档案的强大功能是能够将个人的各种片段及其行为组合在一起。  此聚合称为客户的“联合视图”。  在以下步骤中，您会预览为实时客户配置文件配置的每个XDM类的合并外观
...
>[!NOTE]
>
>只有XDM Individual Profile-based类执行类似命名的字段之间的合并。
