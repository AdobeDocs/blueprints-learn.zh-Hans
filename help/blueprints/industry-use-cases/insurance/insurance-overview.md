...
基于生命周期阶段的产品选项提高了生命周期阶段产品的采用率，在关键决策时刻深化了客户关系。

### 实施方式

将[跨渠道历程与Decisioning](/help/blueprints/use-case-patterns/campaign-management-orchestration/cross-channel-journey-with-decisioning.md)模式一起使用。 生命周期阶段的过渡受益于跨渠道编排与实时决策，从而选择最相关的产品并在最佳时刻通过客户的首选渠道交付产品。 当历程必须跨多个渠道协调交付以确保一致的优惠，同时利用决策功能为检测到的生命周期阶段选择最合适的产品时，这是正确的模式 — 仅多步编排无法提供保险产品推荐所需的实时合格性和适宜性评估。
