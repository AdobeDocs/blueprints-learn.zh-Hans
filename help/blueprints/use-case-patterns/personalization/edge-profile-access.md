...
>[!NOTE]
>
>Edge配置文件访问专为高吞吐量、低延迟用例而设计，例如Web/移动入站个性化和实时Offer Decisioning。 对于吞吐量较低的情况（如代理辅助支持或销售交互），中心配置文件查找API更合适。 查看支持和销售方案的[实时配置文件访问Blueprint](/help/blueprints/use-case-patterns/audience-building-activation/real-time-profile-lookup.md)，了解基于中心的配置文件访问。
