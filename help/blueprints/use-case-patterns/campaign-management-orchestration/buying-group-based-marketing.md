...
- 已在目标沙盒上配置和启用[!DNL AJO B2B Edition]许可证
- 已在目标沙盒上配置和启用[!DNL RT-CDP B2B Edition]许可证
- 可使用适当的API凭据访问CRM系统（[!DNL Salesforce]或[!DNL Microsoft Dynamics 365]）以进行双向数据同步
- 已连接[!DNL Marketo Engage]实例（如果使用[!DNL Marketo]作为营销自动化平台），并配置了源连接器
- 已部署的B2B XDM架构：具有必填字段组的Account、Person、Opportunity、Campaign和Campaign成员类
- 已将帐户和人员数据摄取到AEP，并解决了人员与帐户的关系
- 已配置电子邮件渠道：已委派子域、已预热的IP池，以及为B2B电子邮件发送创建的渠道表面
- 配置的SMS提供程序（如果在帐户历程中使用了SMS渠道）： [!DNL Sinch]、[!DNL Twilio]或配置的[!DNL Infobip]凭据
- 已登记到CRM Sales Insights组件的销售团队（已安装[!DNL Salesforce]个AppExchange包或[!DNL Dynamics]个解决方案）
- 准备了内容资产：用于培养和销售警报电子邮件的B2B电子邮件模板、品牌主题和可视化片段
- 定义的解决方案兴趣分类：将具有关联购买组的产品/服务列表
- 购买设计的小组角色模板：每个解决方案兴趣所需的角色和角色（例如，经济买家、技术评估人员、冠军）
