...
1. 在客户端收集的数据会发送到 Edge 服务器。客户还可以选择首先将数据作为服务器端收集的一种方法发送到自己的服务器。WebSDK 可以提供服务器到服务器的收集功能。但是，这确实需要不同的编程模型来实施。请参阅下面的文档 **Edge 网络服务器 API 概述**

1. Platform Edge 服务器接收数据收集负载并将数据流编排到所需的系统，如 Target 和 Analytics。
