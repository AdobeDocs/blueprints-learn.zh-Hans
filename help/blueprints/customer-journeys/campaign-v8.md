...
了解有关 [Campaign v8部署模型](https://experienceleague.adobe.com/docs/campaign/campaign-v8/config/architecture/architecture.html#ac-deployment){target="_blank"}.
...
| 场景 | 描述 | 功能 |
| :-- | :--- | :--- |
| [[!DNL Real-time Customer Data Platform] 带有Adobe [!DNL Campaign]](rtcdp-and-campaign-v8.md) | 展示Adobe Experience Platform及其Real-time Customer Profile和集中式分段工具如何用于Adobe [!DNL Campaign] 提供个性化的对话 | <ul><li>从共享用户档案和受众 [!DNL Real-Time CDP] 至Adobe [!DNL Campaign] 通过使用cloud storage文件交换和Adobe [!DNL Campaign] 摄取工作流 </li><li>轻松将客户对话中的交付和交互数据共享回 [!DNL Real-Time CDP] 从Adobe [!DNL Campaign] 增强实时客户档案并提供关于消息传送活动的跨渠道报告</li></ul> |
| [[!DNL Journey Optimizer] 带有Adobe [!DNL Campaign]](ajo-and-campaign.md) | 显示如何使用Adobe Journey Optimizer利用Real-time Customer Profile来编排1:1体验并利用本机Adobe [!DNL Campaign] 用于发送消息的事务性消息传递系统 | 利用的实时客户配置文件和功能 [!DNL Journey Optimizer] 在利用Adobe的本机实时消息传送功能的同时编排即时体验 [!DNL Campaign] 进行最后一英里通讯<br><br>注意事项：<br><ul><li>可通过实时消息服务器每小时发送最多 100 万条消息<li>不执行限制 [!DNL Journey Optimizer] 因此，请确保由售前企业架构师进行技术审查</li><li>Campaign v8 的有效负载中不支持决策管理</li></ul> |
...
* Adobe [!DNL Campaign] 客户端控制台需要与 [!DNL Campaign] v8软件。 它是基于 Windows 的客户端，使用标准 Internet 协议（SOAP、HTTP 等）。确保您的组织中已启用分发、安装和运行软件的必要权限
...
* 通过Adobe设置和控制用户访问 [!DNL Admin Console].
* 数据加载到 [!DNL Campaign] 应通过批处理文件完成：
...
* 有三 (3) 种不同类型的短信，它们都受 Adobe 支持：
   * SMS MT（终止的移动设备）：Adobe发出的短信 [!DNL Campaign] 通过SMPP提供商连接到手机。
   * SMS MO（源自移动设备）：由移动设备发送给Adobe的短信 [!DNL Campaign] 通过SMPP提供商。
   * SMS SR（状态报告）或DR或DLR（送达回执）：由移动设备发送给Adobe的回执 [!DNL Campaign] 通过SMPP提供商，指示已成功接收短信。 Adobe [!DNL Campaign] 可能还会收到指示无法投放消息的SR，通常包含错误描述。
