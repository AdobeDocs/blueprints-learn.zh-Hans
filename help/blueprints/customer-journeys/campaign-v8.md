...
| 使用Adobe的[[!DNL Journey Optimizer]  [!DNL Campaign]](ajo-and-campaign.md) | 显示如何使用Adobe Journey Optimizer利用实时客户配置文件编排1:1体验，并利用本机Adobe [!DNL Campaign]事务性消息传递系统发送消息 | 利用[!DNL Journey Optimizer]的实时客户配置文件和强大功能来编排即时体验，同时利用Adobe [!DNL Campaign]的本机实时消息传送功能来执行最后一英里通信<br><br>注意事项：<br><ul><li>可通过实时消息服务器每小时发送最多 100 万条消息<li>没有从[!DNL Journey Optimizer]执行限制，因此请确保由售前企业架构师进行技术审查</li><li>Campaign v8 的有效负载中不支持决策管理</li></ul> |
